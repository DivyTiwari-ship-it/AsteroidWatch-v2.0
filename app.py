...
# Load trained ML Model (XGBoost/RandomForest based on NASA features)
...
def get_planet_position(planet_id):
    ...
    params = {
        ...
        'MAKE_EPHEM': 'YES',
        'EPHEM_TYPE': 'VECTORS',
        'CENTER': '500@10',  # Sun-centered (Solar System Barycenter)
        'START_TIME': start,
        'STOP_TIME': stop,
        'STEP_SIZE': '1d',
        'VEC_TABLE': '2',
...
with tab2:
    ...
    fig.add_trace(go.Scatter3d(
        x=safe['ast_x'], y=safe['ast_y'], z=safe['ast_z'], mode='markers',
...
st.divider()
st.caption("Data Architecture: NASA NeoWs | JPL Horizons API REST System | NASA POWER | NOAA SWPC")            st.metric("Threat Score", f"{threat:.2e}", delta="HIGH RISK")
        else:
            st.success(f"✅ SAFE — {(1-prob)*100:.1f}% confidence")
            st.metric("Threat Score", f"{threat:.2e}", delta="LOW RISK")
